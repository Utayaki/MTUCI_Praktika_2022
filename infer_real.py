...
#35+2 keywords
#CLASSES = 'unknown, silence, backward, bed, bird, cat, dog, down, eight, five, follow, forward, four, go, happy, house, learn, left, marvin, nine, no, off, on, one, right, seven, sheila, six, stop, three, tree, two, up, visual, wow, yes, zero'.split(', ')
#2+2 keywords MTUCI
CLASSES = 'unknown, silence, unknown_word, zdravstvuyte'.split(', ')
...
test_dataloader = data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, pin_memory=True,
                             collate_fn=_collate_fn, num_workers=1)
...
if __name__ == '__main__':
    for epoch in range(epochs):
        model.load_state_dict(torch.load(args.name)["model_state_dict"])
        model.eval()
        ...
        with torch.no_grad():
            ...
            for batch in pbar:
                ...
                pred_batch = pred.cpu()
                y_batch = y.cpu()
                pred_batch = np.array(pred_batch).tolist()
                y_batch = np.array(y_batch).tolist()
                
                pred_full.append(pred_batch)
            ...
            np.save("pred_test_MTUCI.npy", pred_full)
            np.save("y_test_MTUCI.npy", y_full)